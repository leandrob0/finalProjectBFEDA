...
export const playstationTemplate = `<svg
width="24"
height="20"
viewBox="0 0 24 20"
fill="none"
  ...
  return `<article id=${game.id} class="gallery game-container">
    ...
    <div class="second-row">
      <div class="text-container info-key">
        ...
        <p class="info-value genres-value-margin">${(game.genres && game.genres.length !== 0) ? game.genres.map(
          (genre, i) =>
      ...
      ${
        includesPlatform(game, "PlayStation")
          ? playstationTemplate
          : ""
      }
      ${
        includesPlatform(game, "PC")
          ? pcTemplate
          : ""
      }
      ${
        includesPlatform(game, "Xbox")
          ? xboxTemplate
          : ""
      }
      </div>
...
export function cardTemplate(game, index) {
  return `<article id=${game.id} class="card game-container">
    ...
    <div class="third-row">
      ...
      <p class="info-value genres-margin">${(game.genres && game.genres.length !== 0) ? game.genres.map((genre, i) =>
        i + 1 === game.genres.length ? genre.name : genre.name + " "
      ) : 'Unknown'}</p>
    </div>
    </article>`;
